...
package org.computeforcancer.android.rpc;
...
import org.computeforcancer.android.AccountOut;
import org.computeforcancer.android.AcctMgrInfo;
import org.computeforcancer.android.GlobalPreferences;
import org.computeforcancer.android.HostInfo;
import org.computeforcancer.android.Message;
import org.computeforcancer.android.Notice;
import org.computeforcancer.android.Project;
import org.computeforcancer.android.ProjectConfig;
import org.computeforcancer.android.ProjectInfo;
import org.computeforcancer.android.Result;
import org.computeforcancer.android.Transfer;
...
import org.computeforcancer.android.utils.BOINCUtils;
...
public class RpcClient {
	...
	public boolean open(String socketAddress) {
		if (isConnected()) {
			// Already connected
			if(org.computeforcancer.android.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "Attempt to connect when already connected");
		...
		try {
			...
			if(org.computeforcancer.android.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "connect failure: illegal argument", e);
			mSocket = null;
			return false;
		}
		catch (IOException e) {
	...
	public synchronized boolean projectOp(int operation, String projectUrl) {
		try {
			...
			case PROJECT_RESET:
				...
				break;
			default:
				if(org.computeforcancer.android.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "projectOp() - unsupported operation: " + operation);
				return false;
			}
			String request =
				"<" + opTag + ">\n" +
	...
	public synchronized boolean transferOp(int operation, String projectUrl, String fileName) {
		try {
			...
			case TRANSFER_ABORT:
				...
				break;
			default:
				if(org.computeforcancer.android.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "transferOp() - unsupported operation: " + operation);
				return false;
			}
			mRequest.setLength(0);
			mRequest.append("<");
	...
	public boolean resultOp(int operation, String projectUrl, String resultName) {
		try {
			...
			case RESULT_ABORT:
				...
				break;
			default:
				if(org.computeforcancer.android.utils.Logging.LOGLEVEL <= 4) Log.e(Logging.TAG, "resultOp() - unsupported operation: " + operation);
				return false;
			}
			mRequest.setLength(0);
			mRequest.append("<");
